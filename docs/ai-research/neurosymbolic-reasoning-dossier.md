...
| --- | --- | --- | --- | --- |
| Logic Boltzmann Machine (LBM) | Propositional Logic | Energy-Based Learning (Inference via energy minimization) | Type 5: Neuro_{Symbolic} | Logical rules are compiled directly into the fixed weights and biases of a Restricted Boltzmann Machine. |
...
| Logic Tensor Network (LTN) | First-Order Logic (Fuzzy) | Gradient Descent on Satisfiability | Type 5: Neuro_{Symbolic} | Logical formulas are grounded as differentiable operations on tensors; satisfiability becomes a continuous loss function. |
